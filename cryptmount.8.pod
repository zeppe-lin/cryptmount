...
=item B<cryptmount> [I<options>] [B<-L>]

=item B<cryptmount> [I<options>] B<-M> [I<name>|I<device>]

=item B<cryptmount> [I<options>] B<-M> I<name> I<device> [I<key>]

=item B<cryptmount> [I<options>] B<-U> [I<name>[,...]]
...
can also be used to define a I<volume> on the command line without
B<crypttab>.
...
List the I<names> (first column) of I<volumes> from B<crypttab>.  Use
with B<-O> to manipulate which I<names> are listed.

=item B<-M>

Perform a mapping.  The first form of this command finds a I<volume>
from B<crypttab> either by its I<name> or I<device>, or if nothing is
given then map all I<volumes> (use with B<-O> to manipulate which
I<volumes> are mapped).  The second form defines a I<volume> entirely
on the command line, B<crypttab> is not used.

=item B<-U>

Unmap I<volume(s)>.  If a list of I<names> is given, unmap them.
Otherwise unmap all I<volumes> from B<crypttab> (with B<-O> to
manipulate which I<volumes> are unmapped).
...
Similar to I<options>, this is used to manipulate which I<volumes> are
used with I<-L> and with I<-M/-U> without arguments.  If an option is
present in I<filters>, then the I<volume> in B<crypttab> must contain
the option.  For example B<-O swap,size> matches all I<volumes> with
the B<swap> option and any B<size>=size option; or B<-O
swap,size=512> matches only those I<volumes> with the B<swap> option
and specifically B<size=512>.

A filter option may also be negated by prefixing it with a B<!>.  For
example, B<-O swap,!size> includes all I<volumes> with the B<swap>
...
!%random> can be used to map all I<volumes> which don’t use
